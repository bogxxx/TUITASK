...
public class SaveLoadScript : MonoBehaviour
{
    ...
    void Amperage(int i, int j)
    {
        ...
        if (boardColor[i,j] != -1)
        {
            if ((i+1<=13) && (i - 1 >= 0) && (boardColor[i+1, j] == boardColor[i, j] || boardColor[i + 1, j] == -6 || boardColor[i + 1, j] == -3) && !isVisited[i+1, j])
            {
                Amperage(i + 1, j);
            }
            if ((i + 1 <= 13) && (i - 1 >= 0) &&(boardColor[i - 1, j] == boardColor[i, j] || (boardColor[i - 1, j] == -6 || boardColor[i + 1, j] == -3)) && !isVisited[i - 1, j])
            ...
            if ((j + 1 <= 10) && (j - 1 >= 0) && (boardColor[i, j + 1] == boardColor[i, j] || (boardColor[i, j + 1] == -6 || boardColor[i + 1, j] == -3)) && !isVisited[i, j + 1])
            {
                Amperage(i, j + 1);
            }
            if ((j + 1 <= 10) && (j - 1 >= 0) && (boardColor[i, j - 1] == boardColor[i, j] || boardColor[i, j - 1] == -6 || boardColor[i + 1, j] == -3)  && !isVisited[i, j - 1])
    ...
    void OnGUI()
    {
        ...
        if (info)
        {
            ...
            Debug.Log(matrix[xPosEds, yPosEds, 8] + " " +  answerMatrix[xPosEds, yPosEds]);
    ...
    private void Start()
    {
        for (int i = 0; i < 14; i++)
        {
            for (int j = 0; j < 11; j++)
            {
                ...
                resistorsMatrixArr[i, j].secondUzel = 0;
                resistorsMatrixArr[i, j].firstUzel = 0;
                resistorsMatrixArr[i, j].resistorNum = 0;
